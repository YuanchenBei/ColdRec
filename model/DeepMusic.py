...
class DeepMusic(BaseColdStartTrainer):
    def __init__(self, args, training_data, warm_valid_data, cold_valid_data, all_valid_data,
        ...
        super(DeepMusic, self).__init__(args, training_data, warm_valid_data, cold_valid_data, all_valid_data,
                                        warm_test_data, cold_test_data, all_test_data, user_num, item_num,
                                        warm_user_idx, warm_item_idx, cold_user_idx, cold_item_idx, device,
                                        user_content=user_content, item_content=item_content)
        self.model = DeepMusic_Encoder(args, self.data, self.emb_size, device)
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                if self.args.cold_object == 'item':
                    ...
                    batch_loss = mse_loss(pos_item_emb, pos_item_content_emb) + l2_reg_loss(self.reg, pos_item_content_emb)/self.batch_size
                else:
                    pos_user_emb = rec_user_emb[user_idx]
                    pos_user_content_emb = model.generate_user_emb(user_idx)
                    batch_loss = mse_loss(pos_user_emb, pos_user_content_emb) + l2_reg_loss(self.reg, pos_user_content_emb)/self.batch_size
